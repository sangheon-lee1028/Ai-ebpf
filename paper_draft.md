...
|------|------|
| OS | Ubuntu 22.04 LTS |
| 커널 | Linux 6.8.0-138-generic (BTF 지원) |
| CPU | 2 vCPU |
| 메모리 | 4 GB |
...
- **Phase 2**: 취약 디버그 엔드포인트(`/api/v2/predict`) 접근 (L7 우회) — 본 엔드포인트는 L7 우회 시나리오 재현을 위해 AI 서버에 의도적으로 삽입된 실험용 취약점이며, 실 운용 서버에 일반적으로 존재하는 것이 아니다.
...
- **mmap 기반 탈취 대응**: 현재 kShield는 `do_sys_openat2`만 후킹하므로 파일을 한 번 `open()`한 후 `mmap()`으로 메모리 매핑하는 우회 경로에 대응하지 못한다. LSM 훅 또는 `mmap` kprobe 추가를 통해 이 취약점을 보완할 수 있다.
- **꼬리 지연(tail latency) 측정**: 평균 지연 외에 P99, P999 등 꼬리 지연을 측정하여 eBPF 개입 시 간헐적 지연 스파이크 발생 여부를 확인할 필요가 있다.
- **eBPF 프로그램 자원 사용량 측정**: 처리량·지연 외에 kShield BPF 프로그램의 CPU 사용률 증가분과 BPF map 메모리 사용량을 정량화하면 오버헤드 주장을 더욱 엄밀히 뒷받침할 수 있다.
