#!/usr/bin/env python3
"""
stat_analysis.py  —  반복 측정 + Welch's t-test 통계 분석 도구
논문 실험: 벤치마크를 N회 반복 측정하여 평균±표준편차를 산출하고,
두 비교군 간 Welch's t-test로 통계적 유의성을 검증.

사용법:
    # 1단계: 각 그룹별 반복 측정
    python3 stat_analysis.py --group group1 --runs 5
    python3 stat_analysis.py --group group2 --runs 5
    python3 stat_analysis.py --group group3 --runs 5

    # 2단계: 두 그룹 비교 (t-test)
    python3 stat_analysis.py --compare results/stats_group1.csv results/stats_group3.csv
"""
...
def run_compare(args):
    ...
    def sig(p): return " *** (p<0.001)" if p < 0.001 else (" ** (p<0.01)" if p < 0.01 else (" * (p<0.05)" if p < 0.05 else " ns"))
...
# ── 진입점 ─────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="반복 측정 + Welch's t-test")
    sub = parser.add_subparsers(dest="cmd")

    m = sub.add_parser("measure", help="그룹 반복 측정")
    m.add_argument("--group",  required=True, help="그룹 이름 (예: group1)")
    m.add_argument("--runs",   type=int, default=5, help="반복 횟수 (기본: 5)")
